...
1. **Split orchestration from pure transforms.**
   ...
   - Leave a thin effect layer that invokes these helpers and commits their results back into `setCloudClusterRegistry`, enabling unit tests to exercise the pure transforms with fixture JSON. The effect layer should also own writes to mutable structures (e.g., selecting a new smelter recipe) so transforms like `selectSmelterRecipeFromCounts` can return intent descriptors (`{ nextRecipeKey, justification }`) rather than mutating `structure` objects in place. This reconciles the current behavioural coupling inside `syncFactionCloudClusters` (lines 736-759) / `updateFactionCluster` (lines 696-734) with the new declarative flow.
